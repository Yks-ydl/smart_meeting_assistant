...
async def call_service(client: httpx.AsyncClient, url: str, payload: dict, timeout: float = 60.0):
    ...
    try:
        ...
        print(f"[Gateway] 响应内容: {response.text[:200]}...")  # 只打印前200字符
...
async def ws_pipeline_dir(websocket: WebSocket):
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            summary_task = asyncio.create_task(call_service(client, SERVICES["summary_generate"], {"session_id": session_id, "text": full_text}))
            if enable_actions:
                action_task = asyncio.create_task(call_service(client, SERVICES["translation_actions"], {"session_id": session_id, "text": full_text}))
...
# 仅保留目录批量流式回放 WebSocket Pipeline。
# 其他实时会议、单文件转录、REST 代理和离线 full pipeline 已移除。
